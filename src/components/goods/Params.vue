...
export default {
  ...
  methods: {
    ...
    async getParamsData() {
      ...
      // 如果attr_vals不为空，则用空格进行分割为数组，为空则返回空数组，这一步是为了防止attr_vals为空时返回['']
      res.data.forEach(item => {
        item.attr_vals = item.attr_vals ? item.attr_vals.split(' ') : []
        // 控制标签文本框的显示与隐藏
        item.inputVisible = false
        // 文本框中输入的值，使得每一个属性都有自己的文本框
        item.inputValue = ''
      })
    ...
    // 点击按钮修改参数信息，注意put的请求体参数不用params，这是get的
    editParams() {
    ...
    // 根据id删除对应的参数项,形参用attr_id会报错，不是驼峰命名
